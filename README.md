...
For external integrations, SmartCart uses the Google Gemini API (generateContent endpoint) to generate structured recipes and meal plans. The original plan included Instacart Developer Platform API for grocery cart handoff, but access was denied (requires registered business entity). After briefly exploring Spoonacular (rate-limited to 50 points/day), we built static NutritionDatabase (USDA-sourced) and PriceDatabase (Canadian grocery prices) with zero external dependency. All API communication occurs via HTTP/HTTPS calls from the Spring Boot backend.

Scope of the Project
The scope of SmartCart includes building a secure browser-based application with login functionality, structured meal planning workflows, grocery list aggregation, and external REST API integration. Users can register, log in, configure dietary preferences, generate weekly meal plans, view recipes with nutrition and cost data, access consolidated grocery lists with pantry subtraction, swap ingredients at the store, save favourite recipes, and browse past meal plans.
The project includes backend logic, database design, frontend interfaces, API integration, deployment configuration, and validation mechanisms.
...
The fourth epic was originally Instacart API Integration and Cart Handoff, but API access was denied (requires registered business). This was replaced by Grocery Intelligence: per-recipe nutrition data from a USDA-sourced database, Canadian grocery price estimation, store-friendly quantity conversion, and ingredient swap suggestions at the store.
...
- External APIs: Google Gemini (generateContent)
- Static Databases: USDA NutritionDatabase, Canadian PriceDatabase (replaced Instacart/Spoonacular)
